...
class AotwManager
  ...
  validUrl: (url) ->
    ...
    iTunes = /^https?:\/\/(?:www\.)?itunes.apple.com\/[a-zA-Z]{2}\/album\/(?:\S+)?$/
    url.match(spotify) || url.match(googlePlay) || url.match(youtube) || url.match(soundCloud) || url.match(bandCamp) || url.match(iTunes)
